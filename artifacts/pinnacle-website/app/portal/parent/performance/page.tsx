...
import { parents, students, studentTestResults, batches, users } from "@workspace/db/schema";
import { eq, and, asc } from "drizzle-orm";
import { TrendingUp, AlertCircle, Award, CheckCircle, XCircle } from "lucide-react";
import { PerformanceLineChart } from "./PerformanceLineChart";

export const metadata = { title: "Child Performance — Parent Portal" };
...
export default async function ParentPerformancePage() {
  ...
  const dbResults = studentInfo?.id
    ? await db
        .select({
        ...
        .where(eq(studentTestResults.studentId, studentInfo.id))
        .orderBy(asc(studentTestResults.examDate))
    : [];

  const isLinked = !!parentRecord?.studentId && !!studentInfo;
  const isDemo = dbResults.length === 0 && isLinked;
  const data = dbResults.length > 0 ? dbResults : isLinked ? DEMO_RESULTS : [];
    ...
    <div className="space-y-6">
      <div>
        <h1 className="font-[family-name:var(--font-playfair)] text-2xl font-bold text-[var(--color-navy)]">
          {childName ? `${childName}'s Performance` : "Child Performance"}
        </h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            {bestResult && (
              <div className="card border-l-4 border-l-[var(--color-gold)]">
                <div className="text-3xl font-bold text-[var(--color-maroon)] font-[family-name:var(--font-playfair)]">
                  {Math.round((bestResult.marksObtained / bestResult.totalMarks) * 100)}%
                </div>
      ...
      {isLinked && data.length === 0 && !isDemo && (
        <div className="card text-center py-12">
          <TrendingUp size={36} className="text-slate-300 mx-auto mb-3" />
          <p className="text-slate-500">No results published yet. Check back after the next test.</p>
        </div>
      )}
    </div>
  );
