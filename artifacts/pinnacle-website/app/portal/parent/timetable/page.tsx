          ...
          return (
            <div key={day} className="card">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-12 h-12 bg-[var(--color-teal)] rounded-xl flex items-center justify-center text-white font-bold text-sm">{day}</div>
              ...
              <div className="space-y-2">
                {slots.map((s, i) => (
                  <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 bg-[var(--color-slate-light)] rounded-xl">
                    <div className="flex items-center gap-2 text-xs text-slate-500 w-36 flex-shrink-0"><Clock size={12} />{s.time}</div>
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-1">
                        <span className="text-xs text-slate-500 flex items-center gap-1"><User size={11} />{s.teacher}</span>
                        <span className="text-xs text-slate-500 flex items-center gap-1"><MapPin size={11} />{s.room}</span>
                      </div>
                    </div>
                  </div>
                ))}
