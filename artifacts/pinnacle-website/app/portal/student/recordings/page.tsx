...
export default async function RecordingsPage() {
  ...
  const recordings = enrollment?.batchId
    ? await db
        .select({ id: classRecordings.id, title: classRecordings.title, subject: classRecordings.subject, teacherName: classRecordings.teacherName, durationMinutes: classRecordings.durationMinutes, recordingUrl: classRecordings.recordingUrl, viewCount: classRecordings.viewCount, createdAt: classRecordings.createdAt })
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          {enrollment ? `${recordings.length} recording${recordings.length !== 1 ? "s" : ""} available · ${batchName ?? ""}` : "Not enrolled in a batch"}
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {recordings.map((r) => {
          ...
          return (
            <div key={r.id} className="card group hover:shadow-elevated transition-all cursor-pointer p-0 overflow-hidden">
              ...
              <div className="p-4">
                ...
                {r.recordingUrl && r.recordingUrl !== "#" ? (
                  <a href={r.recordingUrl} target="_blank" rel="noopener noreferrer" className="mt-3 text-xs font-semibold text-[var(--color-teal)] hover:underline flex items-center gap-1">
                    Watch Recording
                  </a>
                ) : (
