  ...
  async function addQuestion(e: React.FormEvent) {
    ...
    if (questionType === "mcq" || questionType === "multi") {
      ...
      if (questionType === "multi" && correctOptions.size === 0) {
        setError("Pick at least one correct option."); return;
      }
    } else {
      const n = Number(form.numericalAnswer);
      if (form.numericalAnswer.trim() === "" || !Number.isFinite(n)) { setError("Numerical answer must be a number."); return; }
    ...
    else if (questionType === "multi") Object.assign(payload, {
      optionA: form.optionA, optionB: form.optionB, optionC: form.optionC, optionD: form.optionD,
    ...
    <div className="card">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Paste CSV or upload a .csv file. Required: <code>questionText</code>. For MCQ also <code>optionA-D + correctOption</code>. For multi: <code>questionType=multi</code> + <code>correctOptions=A|C</code>. For numerical: <code>questionType=numerical</code> + <code>numericalAnswer</code> (optional <code>numericalTolerance</code>). Other optional: topic, explanation, image columns. Wrap math with <code>$…$</code>.</p>
        </div>
