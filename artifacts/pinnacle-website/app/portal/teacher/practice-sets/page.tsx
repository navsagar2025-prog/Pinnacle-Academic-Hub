...
export const metadata = { title: "Practice Sets — Teacher Portal" };

export default async function TeacherPracticeSetsPage() {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Pick questions from the bank, bundle them into a set, and assign to your batches.</p>
        </div>
        <CreatePracticeSetButton basePath="teacher" />
      </div>
      ...
      {sets.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {sets.map((s) => (
            <Link key={s.id} href={`/portal/teacher/practice-sets/${s.id}`}
