...
type TestResult = { name: string; date: string; subject: string; score: number; maxScore: number; rank: number };
...
function AnimatedChartBar({
  ...
  useEffect(() => {
    barH.value = withDelay(delay, withTiming((CHART_HEIGHT - 20) * (percentage / 100), { duration: 750 }));
  }, [percentage, delay, barH]);

  const barStyle = useAnimatedStyle(() => ({ height: barH.value }));

  return (
    <View style={[styles.barCol, { height: CHART_HEIGHT }]}>
      <Text style={[styles.barPct, { color }]}>{pctLabel}</Text>
      <View style={[styles.barTrack, { backgroundColor: colors.muted, height: CHART_HEIGHT - 20 }]}>
...
export default function StudentResults() {
  ...
  const scoreColor = (p: number) => (p >= 80 ? colors.success : p >= 60 ? colors.secondary : colors.warning);

  const bestScore = Math.max(...tests.map((t) => pct(t.score, t.maxScore)));
  const avgScore = Math.round(tests.reduce((a, t) => a + pct(t.score, t.maxScore), 0) / tests.length);

  const fetchFn = useCallback(async () => { await new Promise((r) => setTimeout(r, 600)); }, []);
      ...
      <View style={{ marginTop: 16 }}>
        ...
        >
          {FILTERS.map((f) => (
            <TouchableOpacity
              ...
              style={[styles.filterBtn, { backgroundColor: filter === f ? colors.primary : colors.muted, borderRadius: colors.radius - 4 }]}
            >
        ...
        {filtered.map((t, i) => {
          ...
          return (
            <View
              key={i}
              style={[styles.testRow, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}
            >
              <View style={[styles.rankBadge, { backgroundColor: bc + "18", borderRadius: 6 }]}>
                <Text style={[styles.rankText, { color: bc }]}>#{t.rank}</Text>
              </View>
              <View style={{ flex: 1, gap: 4 }}>
                <Text style={[styles.testName, { color: colors.foreground }]}>{t.name}</Text>
                <Text style={[styles.testMeta, { color: colors.mutedForeground }]}>{t.subject} · {t.date}</Text>
                <View style={[styles.scoreBarBg, { backgroundColor: colors.muted }]}>
                  <AnimatedBar percentage={p} color={bc} delay={i * 100} />
                </View>
              </View>
              <View style={styles.scoreCol}>
                <Text style={[styles.scoreMain, { color: bc }]}>{t.score}</Text>
...
function AnimatedBar({ percentage, color, delay }: { percentage: number; color: string; delay: number }) {
  ...
  useEffect(() => {
    if (cw > 0) bw.value = withDelay(delay, withTiming(cw * (percentage / 100), { duration: 700 }));
  }, [cw, percentage, delay, bw]);
  const barStyle = useAnimatedStyle(() => ({ width: bw.value }));
  return (
    <View onLayout={(e) => setCw(e.nativeEvent.layout.width)} style={{ height: 4, backgroundColor: colors.muted, borderRadius: 3, overflow: "hidden" }}>
      <Animated.View style={[{ height: 4, backgroundColor: color, borderRadius: 3 }, barStyle]} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  chartCard: { borderWidth: 1, padding: 14 },
  chartBars: { flexDirection: "row", alignItems: "flex-end", gap: 6, paddingBottom: 4 },
  barCol: { flex: 1, alignItems: "center", gap: 4 },
  barPct: { fontSize: 9, fontWeight: "700" },
  barTrack: { width: "100%", borderRadius: 3, overflow: "hidden", justifyContent: "flex-end" },
  barFill: { width: "100%", borderRadius: 3 },
  barLabel: { fontSize: 9 },
  chartLegend: { flexDirection: "row", flexWrap: "wrap", gap: 10, marginTop: 12, paddingTop: 12, borderTopWidth: 1 },
  legendItem: { flexDirection: "row", alignItems: "center", gap: 5 },
  legendDot: { width: 8, height: 8, borderRadius: 4 },
  legendText: { fontSize: 10 },
  filterBtn: { paddingHorizontal: 14, paddingVertical: 7 },
  filterText: { fontSize: 12, fontWeight: "600" },
  testRow: { flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, padding: 12, marginBottom: 8 },
  ...
  scoreBarBg: { height: 4, borderRadius: 3, overflow: "hidden" },
