...
async function callExportApi(endpoint: string, text: string): Promise<Blob> {
  const response = await fetch(`${BASE_URL}${endpoint}`, {
    ...
    body: JSON.stringify({ text, title: "Scanned Document — Pinnacle Admin" }),
  });
...
export default function AdminScan() {
  ...
  const handleExport = async (type: "pdf" | "docx") => {
    ...
    try {
      ...
      await saveAndShare(blob, `pinnacle-admin-scan.${type}`);
    } catch (err) {
    ...
    if (Platform.OS === "web") {
      return (
        <View style={{ flex: 1, backgroundColor: colors.background }}>
          ...
          <View style={[styles.webFallback, { backgroundColor: colors.maroon + "08", borderColor: colors.maroon + "30", borderRadius: colors.radius, margin: 16 }]}>
            <View style={[styles.scanIconWrap, { backgroundColor: colors.maroon + "15", borderRadius: 48 }]}>
              <Feather name="camera" size={48} color={colors.maroon} />
            </View>
            <Text style={[styles.webTitle, { color: colors.foreground, fontFamily: "PlayfairDisplay_700Bold" }]}>
              Admin Document Scanner
            </Text>
            <Text style={[styles.webSub, { color: colors.mutedForeground }]}>
              Camera viewfinder is available on iOS and Android
            </Text>
            <TouchableOpacity onPress={pickFromGallery} activeOpacity={0.8} style={[styles.actionBtn, { backgroundColor: colors.maroon, borderRadius: colors.radius }]}>
              <Feather name="image" size={18} color="#FFFFFF" />
              <Text style={[styles.actionBtnLabel, { color: "#FFFFFF" }]}>Choose from Gallery</Text>
            </TouchableOpacity>
          </View>
        </View>
      );
    ...
    if (!permission.granted) {
      return (
        <View style={{ flex: 1, backgroundColor: colors.background, justifyContent: "center", padding: 32, gap: 16 }}>
          ...
          <Text style={{ fontSize: 18, fontWeight: "700", color: colors.foreground, textAlign: "center", fontFamily: "PlayfairDisplay_700Bold" }}>
            Camera Access Required
          </Text>
          <Text style={{ fontSize: 14, color: colors.mutedForeground, textAlign: "center" }}>
            Allow camera access to scan documents with the in-app viewfinder.
          </Text>
          <TouchableOpacity onPress={requestPermission} style={[styles.actionBtn, { backgroundColor: colors.maroon, borderRadius: colors.radius }]}>
            <Text style={[styles.actionBtnLabel, { color: "#FFFFFF" }]}>Grant Camera Access</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={pickFromGallery} style={[styles.actionBtn, { backgroundColor: colors.muted, borderRadius: colors.radius }]}>
            <Feather name="image" size={18} color={colors.foreground} />
            <Text style={[styles.actionBtnLabel, { color: colors.foreground }]}>Choose from Gallery</Text>
          </TouchableOpacity>
        </View>
      );
    }
        ...
        <CameraView ref={cameraRef} style={StyleSheet.absoluteFill} facing="back">
          <View style={[styles.viewfinderOverlay, { paddingTop: topPad }]}>
            <View style={styles.viewfinderHeader}>
              <Text style={styles.viewfinderTitle}>Admin Scanner</Text>
            ...
            <View style={styles.cropGuideArea}>
              <View style={[styles.cropBorder, { borderColor: "#8B1A1A" }]}>
                <View style={[styles.cropCorner, styles.cropTL, { borderColor: "#8B1A1A" }]} />
                <View style={[styles.cropCorner, styles.cropTR, { borderColor: "#8B1A1A" }]} />
                <View style={[styles.cropCorner, styles.cropBL, { borderColor: "#8B1A1A" }]} />
                <View style={[styles.cropCorner, styles.cropBR, { borderColor: "#8B1A1A" }]} />
      ...
      <View style={{ flex: 1, backgroundColor: "#000" }}>
        ...
        <View style={[styles.cropOverlay, { paddingTop: topPad, paddingBottom: insets.bottom + 16 }]}>
          ...
          <View style={styles.cropActions}>
            ...
            <TouchableOpacity onPress={runOcr} style={[styles.cropBtn, { backgroundColor: "#8B1A1A" }]}>
              <Feather name="zap" size={18} color="#FFFFFF" />
              <Text style={styles.cropBtnText}>Extract Text</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
  ...
  if (step === "processing") {
    return (
      <View style={{ flex: 1, backgroundColor: colors.background, justifyContent: "center", alignItems: "center", gap: 16 }}>
        ...
        <Text style={{ fontSize: 15, color: colors.mutedForeground, fontFamily: "PlusJakartaSans_500Medium" }}>
  ...
  if (step === "error") {
    return (
      <View style={{ flex: 1, backgroundColor: colors.background, padding: 24, justifyContent: "center", gap: 16 }}>
        ...
        <TouchableOpacity onPress={reset} activeOpacity={0.8} style={[styles.actionBtn, { backgroundColor: colors.maroon, borderRadius: colors.radius }]}>
          <Feather name="refresh-ccw" size={18} color="#FFFFFF" />
          <Text style={[styles.actionBtnLabel, { color: "#FFFFFF" }]}>Try Again</Text>
        </TouchableOpacity>
      </View>
    );
  }
      ...
      <View style={{ flex: 1, backgroundColor: colors.background }}>
        ...
        <View style={{ paddingHorizontal: 16, paddingBottom: insets.bottom + 100, gap: 10, paddingTop: 12 }}>
          <TouchableOpacity
            ...
            disabled={exporting}
            activeOpacity={0.8}
            style={[styles.actionBtn, { backgroundColor: exporting ? colors.muted : colors.maroon, borderRadius: colors.radius, opacity: exporting ? 0.7 : 1 }]}
          >
            {exporting ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Feather name="file" size={18} color="#FFFFFF" />}
            <Text style={[styles.actionBtnLabel, { color: "#FFFFFF" }]}>Export as PDF</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => handleExport("docx")}
            disabled={exporting}
            activeOpacity={0.8}
            style={[styles.actionBtn, { backgroundColor: colors.muted, borderRadius: colors.radius, opacity: exporting ? 0.7 : 1 }]}
          >
            {exporting ? <ActivityIndicator size="small" color={colors.foreground} /> : <Feather name="file-text" size={18} color={colors.foreground} />}
            <Text style={[styles.actionBtnLabel, { color: colors.foreground }]}>Export as DOCX</Text>
          </TouchableOpacity>
